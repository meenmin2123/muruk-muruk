...
## 시각 토큰
- 크기·굵기·모서리·그림자를 스케일 토큰으로 고정(`frontend/app/globals.css` `:root`).
  리터럴 `border-radius`·`font-size`·`font-weight` 는 0곳(32·48px 두 곳만 스케일 밖).
- 배경 그라디언트·컬러 그림자는 쓰지 않는다. 남은 `linear-gradient` 3곳은
  `--grad-primary`(토큰 정의) · `.check.done`(브랜드 액션) · `.today-add::after`(스크롤 페이드).
...
- 커스텀 카테고리는 이모지·이름·색까지(프리셋의 추천 할 일 목록은 없음).
  단, 커스텀 카테고리로 만든 목표도 라벨·색·디데이 설정이 정상 적용된다(`template(cat, customCats)`).
